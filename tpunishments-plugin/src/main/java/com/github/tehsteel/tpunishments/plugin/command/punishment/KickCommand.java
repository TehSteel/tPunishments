...
public final class KickCommand extends BaseCommand {
	...
	protected void run(final CommandSender sender, final String[] args) {
		...
		final Punishment newPunishment = new Punishment.Builder(null, PunishmentType.KICK, target.getUniqueId())
		...
		try {
			plugin.getPunishmentManager().createPunishment(newPunishment).thenAccept((punishment) -> {
				target.kick(MiniMessage.miniMessage().deserialize(
						Constants.Messages.Kick.MESSAGE
								.replace("%reason%", punishment.getReason())
								.replace("%kickid%", punishment.getPunishmentId().id())
				));
			});
